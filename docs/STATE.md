...
- **Open, and next up:** the conversation still slides under Safari's bottom bar and the pending text
  under the phone's clock. A first attempt at that made it worse and was reverted the same day.
...
- Tap a chain: **nav time** (getting through their phone menu, the part we control), what that costs, and how
  ...
  has Daytime · After 9pm · Spanish, because CVS reads a different menu at night. **Recipes** shows v1, v2 with
  the date each locked. **Review** names the store and its buttons now DO the thing they say.
- **The big fix:** Re-map called a real store and told the chain page nothing. Every call feeds the map now,
  ...
  call becomes the recipe. A call that ends on the ring now counts as the good map it is: it sets nav time,
  it counts as a call, and **Reached Staff shows nothing instead of 0%** until a call actually waits for one.
- **CVS was started over** (07-30): its eleven old calls and rejected recipes are gone, because they were made
  before the new mapping worked. The route it runs was kept. Its next call is its first.
...
- **Nothing to flip.** A check from staging.checkitforme.com goes through the new engine. **Listen for:** it asks
  once and waits · it never beeps the keypad once you are talking · after a hold, if it might be a different
  person, it asks you again instead of talking to you like the first person.
