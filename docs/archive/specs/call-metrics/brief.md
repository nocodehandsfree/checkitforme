...
## Build
1. ~~Wire `chains.avgTreeSeconds` into live calls~~ **DONE — verified in code 2026-07-07** (`connectAtSecFor()` → `placeBridgeCall connectAtSec`; VAD is only the fallback, direct-answer chains guarded).
