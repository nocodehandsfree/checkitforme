...
| `comp-2-saved.png` | **RULED 08-06: BUILD IT.** "don't close the window build the screen that shows that their auto check has been saved and they should have a button there that allows them to jump directly into the auto check homepage where they can see their first check." So: the sheet stays open and flips to this screen, and its button goes straight to the list. **The picture to build from is `comp-2-saved-v2.png`** (`render-saved-v2.mjs`), drawn where it really happens with the CHECK STATUS PAGE behind it; the first one was over the search screen, which is not on this path. The title wording is settled: "Auto-check is on." (owner 08-06, "that's fine"). **The store's real logo tile is on it** (owner 08-06), drawn by the page's own `storeTile`, so the picture uses a real store row out of the store table (Target Ventura Blvd, id 4). Two traps the render script now handles: a stubbed `appApi` answers the store lookup with the alerts blob, and the logo host `logos.fungibles.com` must be relayed as well as staging or the tile falls back to a monogram. |
