...
## CHUNK 1 — ROUNDS 1-4 BUILT (08-01), HANDED OFF for the PM's blind milestone-1 audit
**Fix pass 4 (the milestone gate), all four in:** (F4-1) ONE ROOT CLOSED — Staff's voice is never the store's menu. No handoff moment is stamped once a person is on the line ("sure, one moment" read as the machine handing us on, put a handoff AFTER the person, and their hello re-counted as a menu line); `menuLinesOf` cuts at the EARLIER of handoff/person with the person cut STRICT; the sweep's recording test reads only what was heard before the person; `personLineAtSec` stamps the line that TRIGGERED detection (a hello joined onto an earlier store line can no longer sit under the cut); a reply that tells us about the product is an ANSWER, never being sent away ("over in the toy aisle" durably killed the RIGHT door); `resetChainHistory` frees dead doors, spent asks and losing moves, and reports how many. (F4-2) the proven-door exemption keys the way the LEDGER does (full phrase or its shortened winner), and the ask ledger records the question so spent doors are level-scoped like dead ones (old entries still load). (F4-3) proving calls carry a stage so the carrier-end path can't stamp review; a closed-store run reads skipped so pass 2 re-queues it; the mapper waits past the call's own ceiling. (F4-4) the unread `real` flag is gone — heard-twice lives in ONE place: a condition heard once is not listed.
