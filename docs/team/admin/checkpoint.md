...
- COPY/ICON/SPACING sweep (owner asked for the whole admin, keep going across turns). 6-agent audit found 98 (copy 61, icon 24, spacing 13), tracked in `docs/team/admin/copy-icon-audit.md`. WAVES 1-2: 46/98 applied + shipped (most copy: killed dev-speak/dashes/raw values, real feature names, GA4->Web analytics ID, Live stock feed->Latest finds rail, Chats tiers->Bot answered/unsure/needs a human + deleted the Pending pill; 2 emoji->Lucide clock; 2 shared sub-12px classes bumped). Verified qa-glass 11/0 + Chromium smoke clean (caught 2 apostrophe JS breaks from the bulk pass, fixed). REMAINING 52: icon migration to Lucide (SKIP the nav, owner keeps it), per-page sub-12px + tap targets, copy needing (i) sheets or rework (GTM taxonomy, Add import warning, Designer voice-model dropdown, Kiosk raw JSON, Fun Delta status). Do page by page.
