...
- MAPPING page (Chains) cleaned per owner: picker uses `logoTile` (chains=custom logo, hobby/thrift=type icon, matches the site) not initials; added a store-type filter (Retail/Thrift/Hobby/Kiosk); removed the mapped/unmapped filter + per-row text ("it's All"); replaced the top stat wall with one "% of stores mapped" line; removed the Feedback nav count badge (no dynamic nav). Verified in Chromium + qa-glass 11/0. NEXT: same logo fix on the other admin store lists (page by page).
