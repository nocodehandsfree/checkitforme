...
- **Orphan stores (`chainId` null).** A few rows (e.g. "Burlington Jewelry District") have no chain
  link → **no logo, no chain defaults, no chain tier**. Cause: the row's `chain` name didn't match an
  existing `chains` row at import. Fix: re-link by exact chain name.
- **Missing logos.** The thrift chains (Goodwill / Salvation Army / Savers / Unique) have no
  `public/logos/chains/<slug>.png` yet — needed before Treasure Hunt ships. See `docs/STORE_LOGOS.md`.
- **Ungraded long tail.** Some stores still carry `tier: null` (ranked by distance only). Closing this
  is the scoring follow-up.
