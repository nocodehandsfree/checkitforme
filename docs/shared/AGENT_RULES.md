...
26. **A doc the owner hands you gets committed VERBATIM before you derive anything from it.**
    Commit it to `docs/team/<system>/` byte-for-byte first, then write your build spec citing it by
    its committed path. Four owner specs once lived only in chat while every derived doc cited paths
    that did not exist — nothing the agents dropped could be audited until the owner re-uploaded them.
