...
| 2 | **`SYSTEM_MANUAL.md` §2.2 (dial planes), §2.3 (dynamic variables) and §4 (Delta's clip list) were written 2026-07-10 and have not been audited line by line against the current engine.** The rest of §2–4 was corrected 08-06: every model constant, the verdict second read moving to Groq, and a new §3.5 covering the gatekeeper, dropped/reconnected Charlie, self-healing, the versioned map, and the pretend store. | `SYSTEM_MANUAL.md` audit, 08-05/08-06 | voice-calls |
