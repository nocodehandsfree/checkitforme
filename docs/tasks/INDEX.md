...
| Task | System | Status |
|---|---|---|
| [Auto-checks get their own section](site-auto-checks.md) | site | **BUILT + merged to staging 08-19** (PR #123). All four screens plus the empty list, the edit sheet and the locked row; every run keeps its own record including the days it could not run; the ping is in-stock only; the In stock screen offers it too; Manage Zones renamed to Zones; the comp-account bug fixed. Owner looks at it on staging next |
...
| [Translate shows on an English check when a name has an accent](site-translate-on-accented-name.md) | site | owner 08-19 on staging: Staff said "Larry Vásquez" in an all-English call and the result page offered Translate. `looksForeign` treats one accented letter as proof of Spanish. His rule: the name never decides, only the conversation |
| [Location does not follow you to a new town](site-location-follow-gate.md) | site | owner 08-06, hundreds of miles from home and still shown his home stores. DIAGNOSED, not fixed. **The spec is written: `docs/specs/location-follow/README.md`**, and the owner sequenced the build AFTER auto-checks. He picks one of three ways to answer "the browser said no" before anyone starts |
