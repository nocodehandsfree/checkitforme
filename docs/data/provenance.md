...
| **Scoring / tier (1–5)** | `retailers.tier` (per store) | Chain values stamped from `chain_scores_final.csv`; **kiosk overlay forces 5**; per-store voice-confirm overrides. See `docs/data/scoring.md` | `/pub/stores/near` projection → `checkit.html` tier groups |
...
1. **The importer** — `scripts/import-stores.ts` → `POST /api/stores/import` (`src/stores-import.ts`).
   ...
   the thrift rail, and any refresh land. Full field contract: `docs/data/store-schema.md`.
...
- **Missing logos — 13 chains** (see the **Logos** section above for the full list + audit). Includes the
  ...
  Hallmark. Needs image tooling (sharp / ImageMagick) not in every session; pipeline in `docs/data/store-logos.md`.
...
- **`data/pokemon-vending-import.json` is not git-tracked** (regenerated by
  `scripts/archive/collect-pokemon-vending.mjs`). The reconcile endpoint takes the official IDs in the request
...
> **Logos are now DB-first** (full R2 contract in `docs/data/store-logos.md`): `chainLogoInfo()` returns a chain's
...
## Related docs
- `docs/data/store-schema.md` — the importer's full field contract + hours format + dedupe key.
- `docs/data/scoring.md` — the 1–5 tier rubric (how `retailers.tier` is set).
- `docs/data/store-logos.md` — the logo source of truth (chain → `public/logos/chains/<slug>.png`), who
  renders them, and the processing pipeline. (A store's logo resolves by **chain**, never store name.)
- the kiosk spec in git history — kiosks, the official-list rule, the kiosk call script.
- `docs/shared/API_CONTRACT.md` · `docs/shared/STOCK_AND_GEO_API.md` — the front⇄back interface + stock/geo rails.
