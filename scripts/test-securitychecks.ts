...
function run(extra: Record<string, string>): number {
  ...
  for (const k of ["RAILWAY_ENVIRONMENT", "ADMIN_TOKEN", "SESSION_SECRET", "ELEVENLABS_WEBHOOK_SECRET", "STRIPE_WEBHOOK_SECRET"]) delete base[k];
...
ok(run({ RAILWAY_ENVIRONMENT: "production", SESSION_SECRET: STRONG }) === 1, "missing ADMIN_TOKEN → exit 1 (admin API would be UNAUTHENTICATED)");
ok(run({ RAILWAY_ENVIRONMENT: "production", ADMIN_TOKEN: "t0ken", SESSION_SECRET: "short" }) === 1, "weak SESSION_SECRET (<32) → exit 1");
ok(run({ RAILWAY_ENVIRONMENT: "production", ADMIN_TOKEN: "t0ken" }) === 1, "missing SESSION_SECRET → exit 1");
ok(run({ RAILWAY_ENVIRONMENT: "production", ADMIN_TOKEN: "t0ken", SESSION_SECRET: "dev-insecure-secret-change-me" }) === 1, "the dev placeholder secret → exit 1");

console.log("▶ production: a fully secure config boots (exit 0)");
ok(run({ RAILWAY_ENVIRONMENT: "production", ADMIN_TOKEN: "t0ken", SESSION_SECRET: STRONG }) === 0, "admin token + strong secret → boots, even with webhook secrets unset (warn-only)");
...
ok(run({ ADMIN_TOKEN: "", SESSION_SECRET: "short" }) === 0, "insecure values locally → still exit 0");
