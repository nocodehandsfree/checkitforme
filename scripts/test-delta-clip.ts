...
console.log("\n▶ CHECKS 398 TO 405: THE ADVERT NEVER REACHES HIM, IN SOUND OR IN WORDS (owner's order, 08-19)");
{
  ...
  // line for seven.
  {
    ...
    // 2000ms after their first written words, not on the beat their voice stopped.
    await sleep(2300);
...
console.log("\n▶ CHECK 427'S MOMENT: his words are in, his sound is still being made, and the person is proved");
{
  ...
  quiet(tw, 40);
  // The window closes 2000ms after their first written words; nothing lands before that now.
  await sleep(2300);
...
console.log("\n▶ CHECK 437'S FAULT: one answer per turn, one row per turn, one call to the reader");
{
  ...
  clearInterval(stillTalking);
  quiet(tw, 60);
  await sleep(1600);   // the window is 2000ms from the piece, and 600 of it has already run
  ...
  quiet(tw, 60);
  await sleep(1800);   // the window is 2000ms from the piece, and 400 of it has already run
