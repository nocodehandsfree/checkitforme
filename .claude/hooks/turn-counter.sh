...
# empty. The 250-rounds-at-4MB branch is RETIRED TOO (owner, 08-06): Echo was told to hand off at
# 4MB on rounds alone, and the owner's rule is that NOTHING may raise a full chat before 7MB.
# Fires at 7MB of transcript, or at 400 rounds of work ONLY when the transcript cannot be measured
# at all. Then it rides every message after.
input=$(cat)
...
elif [ "$tsize" -ge 7000000 ] || { [ "$tsize" -eq 0 ] && [ "$w" -ge 400 ]; }; then
