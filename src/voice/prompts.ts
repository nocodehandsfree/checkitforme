...
export function oneTurnShipmentDay(question: string): string {
  ...
  return `If they are out of it, sold out, or don't have it right now, ask EXACTLY ONE question, this one, warmly and in your own natural voice: "${q}". This INCLUDES when they volunteer that more is coming ("we're getting a restock soon", "we should have more this week"): ask this once rather than accepting "soon". Then take whatever they give you, warmly wrap and end_call. NEVER ask a second question and never ask them to narrow it down further.`;
}
