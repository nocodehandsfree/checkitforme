 ...
 */
export const PREMIUM_FOLLOWUP = `If they ALREADY named BOTH the set AND the product type in their answer (e.g. "yeah, the Ascended Heroes tin", "just the 151 booster boxes"), you already have it, so warmly acknowledge ("oh perfect, thank you so much!") and END the call. Otherwise ask about the SET FIRST, in one short line: "oh nice! do you know which set it is?" (or "is it the newest set?"). AFTER they answer the set, ask the product type: "gotcha, are those booster packs or tins?". Always ask the SET before the product type. Ask only for a piece they have NOT already given, and NEVER re-ask something they already said. One short question at a time. If they say "not sure" to either, instantly "no worries, thank you so much, have a good one!" and END. Keep it to these two quick questions at most, then end_call. Do NOT wait in silence.`;
