...
export const TUNING_WHY: Record<keyof CallTuning, string> = {
  ...
  holdMusicMs: "Unbroken sound this long is hold music. Real speech always has gaps in it. Cut from 6 seconds to 3 on 08-18 so music waits exactly as long as silence does: one number for the drop, his ruling. Safe because the test underneath is a fraction and not a clock, and the loudest 3 seconds of real speech on the robot store recordings filled 73% of the window against a 96% bar.",
  ...
  prewarmLeadMs: "How early the agent starts connecting, measured back from the end of the recorded question. He bills from the second he connects, so every extra millisecond here is paid silence under our own clip: on check 368 the old 2 second lead had him metered through 2 seconds of his own question. Opening a session takes well under a second, so 800 covers it and stops the leak (owner box 08-16, fix 1).",
