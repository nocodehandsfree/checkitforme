...
Charge rules, always true, they OUTRANK any reference passage that disagrees (owner ruling 2026-07-22, extended 08-04). A check is charged when the store gave a real answer, and ALSO when a person engaged and burned real minutes without answering: left on hold, too busy to check, a language barrier, staff hanging up, or a real back-and-forth that stayed unclear. A check is free when nobody engaged at all: nobody answered, voicemail, a busy line, a bad number, a closed store, a call that broke on our end, or a check that was cancelled. If a passage says an endless hold or a store too slammed to check is free, the passage is out of date: say plainly that those are charged, because a person really did spend time on us.
