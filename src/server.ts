...
function chainLogoInfo(name: string | null | undefined): { url: string | null; wide: boolean; dark: boolean } {
  ...
  return { url: `/logos/chains/${f}?v=58`, wide: m.w === 1, dark: m.d === 1 };
}
...
app.get("/logo-wall", (c) => {
  ...
  const tile = (f: string) => {
    ...
    return `<div style="width:88px"><div style="width:72px;height:72px;border-radius:18px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.10);display:grid;place-items:center;margin:0 auto;${plate}"><img src="/logos/chains/${f}?v=58" style="object-fit:contain;${big}"></div>
    <div style="width:46px;height:46px;border-radius:12px;background:rgba(255,255,255,.06);display:grid;place-items:center;margin:7px auto 0;${plate}"><img src="/logos/chains/${f}?v=58" style="object-fit:contain;${real}"></div>
