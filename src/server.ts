...
function chainLogoInfo(name: string | null | undefined): { url: string | null; wide: boolean; dark: boolean } {
  ...
  return { url: `/logos/chains/${f}?v=74`, wide: m.w === 1, dark: m.d === 1 };
}
...
app.get("/logo-wall", async (c) => {
  ...
  const tile = (f: string) => {
    ...
    return `<div class="cell" data-type="${esc(info?.type || "Other")}" data-treat="${treatKey(m)}"><div class="ic${cls}"><img src="/logos/chains/${f}?v=74" alt=""></div><div class="nm">${esc(info?.name || pretty(f))}</div></div>`;
  };
